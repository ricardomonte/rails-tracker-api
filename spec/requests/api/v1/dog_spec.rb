...
RSpec.describe Api::V1::DogsController, type: :request do
  ...
  describe 'POST #create' do
    it 'should have status success' do
      post '/api/v1/dogs',
           params: {
             dog: {
               name: 'tester', breed: 'german shepard', kilograms: 5, regular_play_time: 2, owner: @user.id
             }
           }, headers: @headers
      expect(response).to have_http_status(:success)
    end
  ...
  describe 'PATCH #update' do
    it 'should have status success' do
      dog = create(:dog)
      patch "/api/v1/dogs/#{dog.id}", params: { dog: { dog_id: dog.id, regular_play_time: 14 } },
                                      headers: @headers
      expect(response).to have_http_status(:success)
    end
  end
